...
def process_genomics_england(  # noqa: PLR0913
    ...
    genomics_england_df = df.select(
        "id", "targetId", "diseaseId", "score", "studyId", "studyOverview"
    )
    genomics_england_df = genomics_england_df.filter(
        pl.col("targetId").is_in(targets["id"])
    )
    genomics_england_df = genomics_england_df.filter(
        pl.col("diseaseId").is_in(disease_phenotype_ids["id"])
    )
    genomics_england_df = genomics_england_df.filter(pl.col("score") > SCORE_THRESHOLD)
