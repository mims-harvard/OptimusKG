...
def process_orphanet(  # noqa: PLR0913
    ...
    orphanet_df = orphanet_df.filter(pl.col("targetId").is_in(targets["id"]))
    orphanet_df = orphanet_df.filter(
        pl.col("diseaseId").is_in(disease_phenotype_ids["id"])
    )
    orphanet_df = orphanet_df.filter(pl.col("score") > SCORE_THRESHOLD)
