...
def run(
    ...
    return (
        ...
        .agg(
            [
                pl.lit(Edge.format_label(Node.BIOLOGICAL_PROCESS, Node.GENE)).alias(
                    "label"
                ),
...
biological_process_gene_node = node(
    run,
    inputs={
        "target": "bronze.opentargets.target",
    },
    outputs="edges.biological_process_gene",
    name="biological_process_gene",
    tags=["silver"],
)
