...
def run(
    ...
    merged = (
        opentargets_phenotype_gene.unique(subset=["from", "to"])
        .join(disgenet_phenotype_gene, on=["from", "to"], how="left")
        .select(
            [
                pl.col("from"),
                pl.col("to"),
                pl.lit(Edge.format_label(Node.PHENOTYPE, Node.GENE)).alias("label"),
                pl.col("relation"),
