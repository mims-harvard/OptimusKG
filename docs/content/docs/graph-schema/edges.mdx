...
```python
{
    "from":       str,   # source node ID (CURIE format)
    "to":         str,   # target node ID (CURIE format)
    "label":      str,   # edge type label (e.g. DIS-GEN)
    "relation":   str,   # relation type (e.g. ASSOCIATED_WITH)
    "undirected": bool,  # if True, edge has no intrinsic directionality
    "properties": str,   # JSON-encoded edge-specific properties
}
```
...
<Cards>
  <Card title="Node Types" href="/docs/graph-schema/nodes" />
