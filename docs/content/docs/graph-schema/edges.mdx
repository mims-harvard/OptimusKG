...
export const genGenProperties = [
  { name: "interaction_type", type: "String", description: "`PHYSICAL_PROTEIN_PROTEIN_INTERACTION` where the pair is supported by a curated physical interaction, otherwise `FUNCTIONAL`." },
  { name: "weight", type: "Float64", description: "GeneMANIA combined association score. `null` on pairs contributed only by the protein-protein interaction source." },
  sourcesField,
];

export const genGenFields = baseEdgeFields("GEN-GEN", true, genGenProperties);
...
| --- | --- | --: |
| `GEN-GEN` | `INTERACTS_WITH` | 11,824,690 |
