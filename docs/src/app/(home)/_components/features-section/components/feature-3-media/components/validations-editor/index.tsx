...
import type { EditorTab, TabbedEditorHandle } from "../../../tabbed-editor";
...
const TREE_DATA: Record<string, TreeNode> = {
  ...
  ...Object.fromEntries(VALIDATIONS.map((v) => [v.id, { name: v.label } satisfies TreeNode])),
};

function validationToTab(v: Validation): EditorTab {
  return {
    name: v.tabName,
    content: <ThemedSvgTabContent alt={v.alt} darkSrc={v.darkSrc} lightSrc={v.lightSrc} />,
  };
}
...
export function ValidationsEditor() {
  ...
  const defaultTabs = VALIDATIONS.filter((v) => DEFAULT_OPEN_IDS.includes(v.id)).map(
    validationToTab,
  );
              ...
              return (
                <TreeItem item={item} key={item.getId()}>
                  <TreeItemLabel
                    className={
                      isRoot
                        ? "font-semibold text-[11px] text-fd-muted-foreground uppercase tracking-[0.04em]"
                        : undefined
                    }
                  >
                    {!item.isFolder() && (
