...
_RATING_STRONG = _ALL_RATINGS[3]
_RATING_VERY_STRONG = _ALL_RATINGS[4]
...
def _by_prevalence(df: pl.DataFrame, col: str) -> list[str]:
    ...
    return (
        df.group_by(col).len().sort("len", descending=True)[col].drop_nulls().to_list()
    )
...
def _node_types_by_pct_delta(df: pl.DataFrame) -> list[str]:
    ...
    counts = true_df.group_by("seed_node_type", "rating").len().rename({"len": "count"})
    totals = true_df.group_by("seed_node_type").len().rename({"len": "total"})
    ...
    for nt in node_types:
        ...
        c5 = counts.filter(
            (pl.col("seed_node_type") == nt) & (pl.col("rating") == _RATING_VERY_STRONG)
        )
        c2 = counts.filter(
            (pl.col("seed_node_type") == nt) & (pl.col("rating") == _RATING_WEAK)
        )
...
def _plot_barplot(df: pl.DataFrame, out_dir: Path, run_id: str) -> None:
    ...
    for ax, (is_true, panel_title) in zip(
        axes, [(False, "False edges"), (True, "True edges")]
    ):
...
def _print_stats(df: pl.DataFrame) -> None:  # noqa: PLR0915
    ...
    logger.info(sep)
    logger.info("  PaperQA evaluation summary")
    logger.info(sep)
    logger.info("  Total queries : %s", f"{total:,}")
    ...
    for is_true, label in [(True, "True edges"), (False, "False edges")]:
        ...
        rating_counts = sub.group_by("rating").len().sort("rating")

        any_evidence = sub.filter(pl.col("rating") > _RATING_NO_EVIDENCE).height

        logger.info("")
        logger.info("  %s  (n=%s)", label, f"{n:,}")
        for row in rating_counts.iter_rows(named=True):
            r, cnt = row["rating"], row["len"]
            label_str = (
                _RATING_LABELS[r - 1] if _RATING_MIN <= r <= _RATING_MAX else str(r)
            )
            pct = cnt / n * 100
            logger.info(
                "    Rating %s (%-14s) : %5s  (%5.1f%%)",
                r,
                label_str,
                f"{cnt:,}",
                pct,
    ...
    for rating_val, heading in [
        (_RATING_NO_EVIDENCE, "True edges with NO evidence (rating = 1)"),
        (
            _RATING_VERY_STRONG,
            "True edges with VERY STRONG evidence (rating = 5)",
        ),
    ]:
        ...
        rel_counts = sub.group_by("relation_type").len().sort("len", descending=True)

        logger.info("")
        logger.info(
            "  %s  (n=%s / %.1f%% of true edges)",
            heading,
            f"{n_sub:,}",
            n_sub / n_true * 100,
        )
        for row in rel_counts.iter_rows(named=True):
            ...
            logger.info(
                "    %-30s : %5s  (%5.1f%% of rating=%s | %5.1f%% of all true)",
                str(rel),
                f"{cnt:,}",
                pct_of_sub,
                rating_val,
    ...
    logger.info("")
    logger.info(sep)
    logger.info("  Per-node-type summary")
    logger.info(sep)
    # for node_type in _by_prevalence(df, "seed_node_type"):
    node_types = _node_types_by_pct_delta(df)
    for node_type in node_types:
        nt_df = df.filter(pl.col("seed_node_type") == node_type)
        true_nt = nt_df.filter(pl.col("is_true_edge"))
        logger.info("  %s: %5s", node_type, f"{nt_df.height:,}")
        logger.info("    True edges: %5s", f"{true_nt.height:,}")
        logger.info(
            "    False edges: %5s",
            f"{nt_df.filter(~pl.col('is_true_edge')).height:,}",
        )
        logger.info("    Total edges: %5s", f"{nt_df.height:,}")
        logger.info(
            "    Weak evidence: %5s",
            f"{true_nt.filter(pl.col('rating') == _RATING_WEAK).height:,}",
        )
        logger.info(
            "    Moderate evidence: %5s",
            f"{true_nt.filter(pl.col('rating') == _RATING_MODERATE).height:,}",
        )
        logger.info(
            "    Strong evidence: %5s",
            f"{true_nt.filter(pl.col('rating') == _RATING_STRONG).height:,}",
        )
        logger.info(
            "    Very strong evidence: %5s",
            f"{true_nt.filter(pl.col('rating') == _RATING_VERY_STRONG).height:,}",
        )
        denominator = true_nt.height
        if denominator > 0:
            with_evidence = true_nt.filter(pl.col("rating") >= _RATING_WEAK).height
            logger.info(
                "    True edges with evidence: %5.1f%%",
                with_evidence * 100 / denominator,
            )
        else:
